...
function fileIcon(name: string) {
  ...
  return <span className="text-slate-600 dark:text-slate-400"><FileText className="h-8 w-8" /></span>
}
...
export default function PortalFilesPage() {
  ...
  return (
    <div className="p-4 lg:p-8">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Files</h1>
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[1,2,3,4].map(i => <div key={i} className="h-32 bg-slate-900/[0.04] dark:bg-white/[0.04] rounded-xl animate-pulse" />)}
        </div>
      ) : files.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {files.map(file => (
            <div key={file.id} className="glass-card p-4 flex flex-col items-center gap-2 hover:bg-slate-900/[0.03] dark:hover:bg-white/[0.03] transition-all">
              {fileIcon(file.name)}
              <p className="text-xs font-medium text-slate-700 dark:text-slate-300 text-center line-clamp-2">{file.name}</p>
              <p className="text-xs text-slate-600 dark:text-slate-400">{new Date(file.createdTime).toLocaleDateString()}</p>
