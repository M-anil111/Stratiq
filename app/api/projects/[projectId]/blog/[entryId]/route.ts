...
export async function PUT(request: NextRequest, { params }: { params: { projectId: string; entryId: string } }) {
  ...
  const { submission_date, live_url, meta_title, meta_description, h1, username, password, comment } = body

  const { data, error } = await supabase
    .from('blog_submissions')
    .update({ submission_date, live_url, meta_title, meta_description, h1, username, password_encrypted: encryptIfPresent(password), comment, updated_at: new Date().toISOString() })
