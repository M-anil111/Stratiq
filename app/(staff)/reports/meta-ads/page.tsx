...
import { Download, TrendingUp, MousePointer, DollarSign, BarChart2, Percent, Users } from 'lucide-react'
...
export default function MetaAdsReportPage() {
  ...
  const kpis = [
    ...
    { label: 'CTR', value: report?.meta_ctr != null ? fmt(report.meta_ctr, '', '%', 2) : ctr(report?.meta_clicks, report?.meta_impressions), icon: Percent, color: 'text-teal-400', bg: 'bg-teal-500/10' },
  ]
    ...
    <div className="p-4 lg:p-8">
      ...
      {!clientId && (
        <div className="bg-sky-500/10 border border-sky-500/20 rounded-xl p-4 mb-6">
          <p className="text-sky-300 text-sm font-medium">Select a client to view their Meta Ads report</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="glass-card">
            <div className="p-5 border-b border-white/[0.06]">
              <h2 className="font-semibold text-white">Ad Sets</h2>
              <p className="text-sm text-slate-400 mt-0.5">Individual ad set breakdown for this period</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {['Ad Set', 'Reach', 'Impressions', 'Clicks', 'CTR', 'Spend', 'ROAS'].map(h => (
                ...
                <tbody>
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center text-slate-500 text-sm">
                      No ad set data available for this period.<br />
                      <span className="text-xs">Ad set-level data will appear here once synced via the Meta Ads integration.</span>
                    </td>
                  </tr>
                </tbody>
              </table>
