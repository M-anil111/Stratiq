    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Automate report delivery to clients and stakeholders</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {reports.length === 0 ? (
                ...
                <tr key={rep.id} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                  <td className="px-5 py-3.5 font-medium text-white">{rep.client}</td>
                  <td className="px-5 py-3.5 text-slate-300">{rep.type}</td>
                  <td className="px-5 py-3.5">
                    <span className="inline-flex items-center gap-1 text-slate-300">
                      <Clock className="h-3.5 w-3.5 text-slate-500" /> {rep.frequency}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-slate-300">{rep.frequency === 'Monthly' ? `Day ${rep.day}` : rep.day}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex flex-wrap gap-1">
                      {rep.recipients.map(email => (
                        <span key={email} className="inline-flex items-center gap-1 bg-white/[0.07] text-slate-300 text-xs px-2 py-0.5 rounded-full">
                          <Mail className="h-2.5 w-2.5" /> {email}
                        </span>
                      ))}
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-slate-300 whitespace-nowrap">{rep.nextRun}</td>
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full ${rep.status === 'active' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-500/15 text-slate-400'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative glass-card w-full max-w-lg p-6 z-10">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-white">Schedule New Report</h2>
              <button onClick={() => setModalOpen(false)} className="text-slate-400 hover:text-white transition-colors p-1 rounded">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {FREQUENCIES.map(f => (
                    <button key={f} onClick={() => { setFFrequency(f); setFDay(f === 'Weekly' ? DAYS_OF_WEEK[0] : '1') }}
                      className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-all ${fFrequency === f ? 'btn-brand border-transparent' : 'border-white/[0.12] text-slate-300 hover:bg-white/[0.06]'}`}>
                      {f}
                    </button>
                  ))}
                </div>
              ...
              {/* Recipients */}
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1.5">Recipients</label>
                <div className="flex gap-2">
                  <input
                  ...
                  />
                  <button onClick={addEmail} className="px-3 py-2.5 rounded-xl border border-white/[0.12] text-slate-300 hover:bg-white/[0.06] transition-all text-sm whitespace-nowrap">
                    Add
                  </button>
                </div>
                {fRecipients.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {fRecipients.map(email => (
                      <span key={email} className="inline-flex items-center gap-1 bg-white/[0.08] text-slate-300 text-xs px-2.5 py-1 rounded-full">
