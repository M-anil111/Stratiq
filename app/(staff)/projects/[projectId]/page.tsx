  ...
  if (notFound || !project) {
    return (
      <div className="p-6 max-w-5xl mx-auto">
        <Link href="/projects" className="text-sm text-slate-400 hover:text-white inline-flex items-center gap-1 mb-4">
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="glass-card p-6 mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">{project.name || project.domain || 'Untitled project'}</h1>
              <div className="flex items-center gap-3 mt-1 text-sm text-slate-400 flex-wrap">
                {project.domain && <span>{project.domain}</span>}
                {project.client && (
                  <Link href={`/clients/${project.client.id}`} className="inline-flex items-center gap-1 text-slate-300 hover:text-sky-400">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {STATUS_OPTIONS.map((s) => <option key={s} value={s} className="bg-slate-900 text-white">{s.replace(/_/g, ' ')}</option>)}
            </select>
            {savingStatus && <Loader2 size={16} className="animate-spin text-slate-400" />}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          ...
          {project.client && (
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3 col-span-2">
        ...
        {project.notes && (
          <div className="mt-4 text-sm text-slate-300 bg-white/[0.03] rounded-xl p-3 whitespace-pre-wrap">{project.notes}</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="glass-card p-4">
          ...
          <p className="text-2xl font-bold text-white mt-1">{fmt(paid)}</p>
        ...
        {invoices.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id} className="border-b border-white/[0.04] hover:bg-white/[0.03]">
                    <td className="px-4 py-2">
                      <Link href={`/invoices`} className="text-white hover:text-sky-400 font-medium">{inv.invoice_number || inv.id.slice(0, 8)}</Link>
                    </td>
                    <td className="px-4 py-2 text-slate-300">{inv.status}</td>
                    <td className="px-4 py-2 text-slate-400 text-xs">{fmtDate(inv.issue_date)}</td>
                    <td className="px-4 py-2 text-right text-slate-200">{fmt(inv.total)}</td>
                    <td className="px-4 py-2 text-right text-slate-400">{fmt(inv.amount_paid)}</td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="glass-card w-full max-w-lg max-h-[80vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">Add invoice to project</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-white"><X size={20} /></button>
        </div>
        {loading ? (
          <div className="py-8 text-center"><Loader2 size={20} className="animate-spin text-slate-400 mx-auto" /></div>
        ) : invoices.length === 0 ? (
          <p className="py-8 text-center text-sm text-slate-500">No unlinked invoices available.</p>
        ) : (
          <div className="space-y-2">
            {invoices.map((inv) => (
              <div key={inv.id} className="flex items-center justify-between bg-white/[0.04] rounded-xl px-3 py-2.5">
                <div>
                  <p className="text-sm text-white font-medium">{inv.invoice_number || inv.id.slice(0, 8)}</p>
