    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Track projects, assign clients, and see project financials.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="glass-card p-4">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{totalInvoices}</p>
        </div>
      </div>
        ...
        {loading ? (
          <div className="p-4 space-y-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-12 rounded-lg bg-slate-900/[0.04] dark:bg-white/[0.04] animate-pulse" />
            ))}
          </div>
        ) : projects.length === 0 ? (
          <div className="p-12 text-center">
            <Briefcase size={40} className="mx-auto text-slate-600 mb-3" />
            <p className="text-slate-700 dark:text-slate-300 font-medium">No projects yet</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {projects.map((p) => (
                  <tr key={p.id} className="border-b border-slate-900/10 dark:border-white/[0.04] hover:bg-slate-900/[0.04] dark:hover:bg-white/[0.03] transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/projects/${p.id}`} className="font-medium text-slate-900 dark:text-white hover:text-sky-400 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {p.client ? (
                        <Link href={`/clients/${p.client.id}`} className="text-slate-700 dark:text-slate-300 hover:text-sky-400 inline-flex items-center gap-1">
                    ...
                    <td className="px-4 py-3 text-right text-slate-700 dark:text-slate-300">{p.invoice_count || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="glass-card w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Project name</label>
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-4 py-2.5 text-sm text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white">Cancel</button>
