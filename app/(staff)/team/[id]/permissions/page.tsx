...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500/50 cursor-pointer ${checked ? 'bg-sky-500' : 'bg-slate-300 dark:bg-slate-700'}`}
    >
...
export default function UserAccessPage() {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-6 space-y-4">
        ...
        <div className="h-20 rounded-lg bg-slate-200 dark:bg-slate-800 animate-pulse" />
        {[0, 1, 2, 3].map(i => <div key={i} className="h-14 rounded-lg bg-slate-200 dark:bg-slate-800 animate-pulse" />)}
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      ...
      <Link href="/team" className="mb-4 inline-flex items-center gap-1 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200">
        <ArrowLeft className="h-4 w-4" /> Back to team
      </Link>

      {/* Header */}
      <div className="mb-6 flex flex-col gap-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-400">Apply role defaults</label>
          <select
            value={presetRole}
            onChange={e => applyRoleDefaults((e.target as HTMLSelectElement).value)}
            className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-1.5 text-sm text-slate-800 dark:text-slate-200 focus:border-sky-500 focus:outline-none"
          >
        ...
        <button onClick={turnOffAll} className="self-start text-sm text-slate-600 dark:text-slate-400 hover:text-red-400 sm:self-auto">
          Turn off all
        </button>
      </div>
      ...
      <div className="space-y-2">
        {resources.map(res => {
          ...
          return (
            <div key={res.key} className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
              <button
              ...
              {isOpen && (
                <div className="border-t border-slate-200 dark:border-slate-800 px-4 py-3">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {res.actions.map(a => (
                      <div key={a} className="flex items-center justify-between rounded-md bg-slate-100 dark:bg-slate-800/40 px-3 py-2">
                        <span className="text-sm text-slate-700 dark:text-slate-300">{ACTION_LABELS[a]}</span>
      ...
      {/* Permission history */}
      <div className="mt-8 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
        <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 px-4 py-3">
          ...
          <h2 className="text-sm font-medium text-slate-900 dark:text-slate-100">Permission history</h2>
        </div>
        {history.length === 0 ? (
            ...
            {history.map(h => (
              <li key={h.id} className="flex flex-col gap-0.5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <span className="text-sm text-slate-700 dark:text-slate-300">
                  Changed by <span className="font-medium text-slate-900 dark:text-slate-100">{h.user_email || 'unknown'}</span>
                </span>
