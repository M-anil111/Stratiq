    ...
    <div>
      <div className="glass-card">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-900/10 dark:border-white/[0.06]">
          <h2 className="font-semibold text-slate-900 dark:text-white">Group Postings ({entries.length})</h2>
          <button
            onClick={openAdd}
              ...
              <tbody className="divide-y divide-slate-900/10 dark:divide-white/[0.06]">
                {entries.map((entry, i) => (
                  <tr key={entry.id} className="hover:bg-slate-900/[0.04] dark:hover:bg-white/[0.03]">
                    ...
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{entry.platform}</td>
                    <td className="px-4 py-3 max-w-[140px] truncate text-slate-700 dark:text-slate-300">{entry.group_name}</td>
                    <td className="px-4 py-3 capitalize text-slate-700 dark:text-slate-300">{entry.post_type.replace('_', ' ')}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass w-full max-w-lg rounded-2xl p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{editEntry ? 'Edit Group Posting' : 'Add Group Posting'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Group Name <span className="text-red-400">*</span></label>
                <input className="input-glass" value={form.group_name} onChange={set('group_name')} placeholder="Group or community name" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Group URL</label>
                <input className="input-glass" type="url" value={form.group_url} onChange={set('group_url')} placeholder="https://..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Post Content <span className="text-red-400">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Live Link</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Member Count</label>
