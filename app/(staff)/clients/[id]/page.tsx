        ...
        return (
          ...
          <div className="glass-card p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="font-semibold text-white">Google Ads Account</h2>
            ...
            {googleAdsError === 'not_connected' ? (
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-4 text-sm text-amber-300">
                ...
                <a href="/settings/integrations" className="underline hover:text-amber-200">Connect Google Account in Settings → Integrations</a>
              </div>
            ) : googleAdsLoading ? (
              ...
              <div className="space-y-3">
                {savedGoogleAccount && (
                  <div className="text-xs text-slate-400">
                    Current: <span className="text-slate-300 font-medium">{savedGoogleAccount.account_name || savedGoogleAccount.account_id}</span>
                  </div>
                )}
                ...
                {savedGoogleAccount && (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const yearAdj = now.getMonth() === 0 ? y - 1 : y
                      const period_start = `${yearAdj}-${String(m).padStart(2, '0')}-01`
                      const lastDay = new Date(yearAdj, m, 0).getDate()
                      const period_end = `${yearAdj}-${String(m).padStart(2, '0')}-${lastDay}`
                      await fetch('/api/integrations/google-ads/sync', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                          client_id: params.id,
                          customer_id: savedGoogleAccount.account_id,
                          period_start,
                          period_end,
                        }),
                      })
                  ...
                  >
                    ...
                    {syncingGoogle ? 'Syncing…' : googleSyncDone ? 'Synced!' : 'Sync Data'}
                  </button>
                )}
              </div>
            )}
