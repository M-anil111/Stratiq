      ...
      <div className="max-w-xl">
        <div className="glass-card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-semibold text-white">Connect Google Analytics</h2>
              <p className="text-xs text-slate-400">Link a GA4 property to view this client&apos;s traffic</p>
            </div>
          </div>
          {notConnected ? (
            ...
            <p className="text-sm text-slate-400">No GA4 properties found for the connected Google account.</p>
          ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {tiles.map(t => (
          <div key={t.label} className="glass-card p-4">
            ...
            <p className="text-xl font-bold text-white truncate">{t.value}</p>
            {t.delta !== null && (
      ...
      <div className="glass-card p-5">
        <h3 className="font-semibold text-white text-sm mb-4">Sessions over time</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-2.5">
            {(data?.trafficSources || []).map((src: any) => (
              <div key={src.label}>
                <div className="flex justify-between text-xs mb-1"><span className="text-slate-300 truncate">{src.label}</span><span className="text-slate-500">{src.sessions.toLocaleString()}</span></div>
                <div className="h-1.5 rounded-full bg-white/[0.06]"><div className="h-full rounded-full bg-sky-400" style={{ width: `${(src.sessions / sourceMax) * 100}%` }} /></div>
              </div>
            ))}
        ...
        {/* Top countries */}
        <div className="glass-card p-5">
          <h3 className="font-semibold text-white text-sm mb-4">Top Countries</h3>
          <div className="space-y-2">
            {(data?.countries || []).map((c: any) => (
              <div key={c.label} className="flex justify-between text-sm"><span className="text-slate-300 truncate">{c.label}</span><span className="text-slate-500">{c.sessions.toLocaleString()}</span></div>
            ))}
        ...
        {/* Devices */}
        <div className="glass-card p-5">
          <h3 className="font-semibold text-white text-sm mb-4">Devices</h3>
          <div className="space-y-3">
            {(data?.devices || []).map((d: any) => (
              <div key={d.label}>
                <div className="flex justify-between text-xs mb-1"><span className="text-slate-300 capitalize">{d.label}</span><span className="text-slate-500">{((d.sessions / deviceTotal) * 100).toFixed(0)}%</span></div>
                <div className="h-1.5 rounded-full bg-white/[0.06]"><div className="h-full rounded-full bg-violet-400" style={{ width: `${(d.sessions / deviceTotal) * 100}%` }} /></div>
              </div>
            ))}
