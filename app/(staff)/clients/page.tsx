...
function StatTile({ label, children, className = '' }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-white/[0.04] border border-white/[0.06] rounded-xl p-4 flex flex-col gap-1 ${className}`}>
        ...
        <div className="px-3 py-2 flex gap-1.5 flex-wrap border-b border-white/[0.06]">
          {STATUS_FILTERS.map(f => (
            <button key={f.value} onClick={() => onStatus(f.value)}
              className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${statusFilter === f.value ? 'bg-sky-500 text-white' : 'bg-white/[0.05] text-slate-400 hover:text-white'}`}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      <div className="hidden lg:block flex-1 overflow-y-auto bg-[#080f1e]">
        {!selectedId ? (
          ...
          <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <StatTile label="Projects">
                  <div className="flex flex-col gap-0.5">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-white font-bold">{openProjects.toString().padStart(2, '0')}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Close</span>
                      <span className="text-white font-bold">{closedProjects.toString().padStart(2, '0')}</span>
                    </div>
                    <div className="flex justify-between text-xs border-t border-white/[0.06] pt-0.5 mt-0.5">
                ...
                <StatTile label="Active Services">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-bold text-white">{pkgs.length}</span>
                  </div>
                </StatTile>
                ...
                <StatTile label="Revenue Level">
                  <div className="flex items-center gap-2">
                    <TrendingUp className={`h-4 w-4 ${monthlyRevenue > 5000 ? 'text-emerald-400' : monthlyRevenue > 2000 ? 'text-amber-400' : 'text-slate-400'}`} />
                    <span className={`text-sm font-bold ${monthlyRevenue > 5000 ? 'text-emerald-400' : monthlyRevenue > 2000 ? 'text-amber-400' : 'text-slate-400'}`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  ...
                  <div className="flex flex-col gap-0.5 text-xs">
                    {pkgs.slice(0, 3).map((p: any, i: number) => (
                      <div key={i} className="flex justify-between">
                        <span className="text-slate-400 truncate max-w-[80px]">{p.service}</span>
                        <span className="text-white font-medium ml-1">${(parseFloat(p.price) || 0).toLocaleString()}</span>
                      </div>
                    ))}
                ...
                <StatTile label="Maint. Since">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-bold text-white">{client.maint_since || '—'}</span>
                  </div>
                </StatTile>

                <StatTile label="Maint. Degree">
                  <span className={`text-sm font-bold capitalize ${client.maint_degree === 'high' ? 'text-rose-400' : client.maint_degree === 'low' ? 'text-slate-400' : 'text-amber-400'}`}>
            ...
            {/* ── PROJECTS TABS ── */}
            <div className="mx-5 mb-5 bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
              <div className="flex items-center border-b border-white/[0.08] overflow-x-auto">
                {projects.length === 0 ? (
                  ...
                  <button key={p.id} onClick={() => setActiveProjectIdx(idx)}
                    className={`flex items-center gap-2 px-4 py-3 text-xs font-medium whitespace-nowrap border-b-2 transition-colors shrink-0
                      ${activeProjectIdx === idx ? 'border-sky-400 text-white bg-sky-500/5' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
                    <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold shrink-0
                      ${activeProjectIdx === idx ? 'bg-sky-500 text-white' : 'bg-white/[0.08] text-slate-400'}`}>
                      {idx + 1}
                    </span>
                    <span className="max-w-[120px] truncate">{p.domain}</span>
                    {p.status && (
                      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium border ${STATUS_COLORS[p.status] || 'bg-slate-500/20 text-slate-400 border-slate-500/30'}`}>
              ...
              <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
                ...
                <InfoRow label="About" value={
                  client.about_company ? (
                    <span className="text-slate-300 text-xs leading-relaxed line-clamp-3">{client.about_company}</span>
                  ) : null
                } />
                {client.proposal_url && (
                ...
                <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
                  <div className="flex border-b border-white/[0.08]">
                    <button onClick={() => setActiveServiceTab('active')}
                      className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${activeServiceTab === 'active' ? 'text-sky-400 bg-sky-500/5 border-b-2 border-sky-400' : 'text-slate-400 hover:text-slate-200'}`}>
                      Active Services
                    </button>
                    <button onClick={() => setActiveServiceTab('expired')}
                      className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${activeServiceTab === 'expired' ? 'text-slate-300 bg-white/[0.03] border-b-2 border-slate-400' : 'text-slate-500 hover:text-slate-300'}`}>
                      Expired Services
                    </button>
                  </div>
                  <div>
            ...
            {(client.hashtags?.length > 0 || client.categories?.length > 0) && (
              <div className="mx-5 mb-5 bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {[...(client.categories || []), ...(client.hashtags || [])].map((tag: string, i: number) => (
                    <span key={i} className="px-3 py-1 rounded-full border border-white/[0.12] text-xs text-slate-300 hover:border-sky-500/40 hover:text-sky-300 transition-colors cursor-default">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            {/* ── TIMELINE ── */}
            <div className="mx-5 mb-6 bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
              ...
              <div className="relative overflow-x-auto">
                <div className="flex items-center gap-0 min-w-max pb-2">
                  ...
                  <div className="flex items-center">
                    <div className="w-16 h-px bg-white/[0.1]" />
                    <div className="flex flex-col items-center mx-4">
                      ...
                      <div className="bg-sky-500/10 border border-sky-500/20 rounded-lg px-3 py-2 text-center mb-1">
                        ...
                        {client.sales_manager && <p className="text-[10px] text-slate-400">by {client.sales_manager.full_name}</p>}
                      </div>
                  ...
                  {projects.map((p: any, i: number) => (
                    <div key={p.id} className="flex items-center">
                      ...
                      <div className="flex flex-col items-center mx-4">
                        ...
                        <div className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-center mb-1 max-w-[140px]">
                ...
                {/* Timeline line */}
                <div className="absolute top-[18px] left-0 right-0 h-px bg-white/[0.08] -z-0" style={{ top: '1rem' }} />
              </div>
            </div>
            ...
            <div className="mx-5 mb-6 flex gap-3 flex-wrap">
              ...
              <a href={`/clients/${client.id}`}
                className="flex items-center gap-2 px-4 py-2 bg-white/[0.04] border border-white/[0.08] text-slate-300 text-sm font-medium rounded-xl hover:bg-white/[0.08] transition-colors">
