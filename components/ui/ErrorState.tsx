...
interface ErrorStateProps {
  ...
  /** Use 'sm' for tight spaces. Defaults to 'md'. */
  size?: 'sm' | 'md'
  className?: string
}

export default function ErrorState({ icon: Icon = AlertCircle, title, description, action, size = 'md', className = '' }: ErrorStateProps) {
  const isSm = size === 'sm'

  return (
    <div className={`${isSm ? 'p-4' : 'p-10'} text-center text-slate-600 dark:text-slate-400 ${className}`}>
      <div className={`mx-auto ${isSm ? 'mb-2 h-8 w-8' : 'mb-3 h-11 w-11'} rounded-xl bg-red-500/10 text-red-500 dark:text-red-400 flex items-center justify-center`}>
